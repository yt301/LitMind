...
async def get_literatures(current_user: User = Depends(
    get_current_user)):
    # 查询该用户的文献记录
    literatures_data = await Literature.filter(users__username=current_user.username).values(
...
async def update_literatures(literature_in: LiteratureIn,
    ...
    literatures_existing = await Literature.filter(doi=literature_in.doi, users__username=current_user.username).first()
    ...
    # 更新文献记录
    await Literature.filter(doi=literature_in.doi, users__username=current_user.username).update(**literature_in.model_dump())
    literature_changed = await Literature.filter(doi=literature_in.doi, users__username=current_user.username).first()
...
async def delete_literatures(doi_in: DOIIn, current_user: User = Depends(get_current_user)):
    # 获取文献记录
    literature = await Literature.filter(doi=doi_in.doi).first()
    if not literature:
        return create_response("error", 404, "文献记录不存在！")
