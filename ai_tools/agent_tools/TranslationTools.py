...
from ai_tools.Prompts import PROMPT_ACADEMIC_TRANSLATE, PROMPT_GENERAL_TRANSLATE, PROMPT_LITERARY_TRANSLATE
...
def general_translate(llm, text: str, source_language: str, translated_language: str, style: str) -> str:
    ...
    processed_input = gain_userinput(userinput=text, source_language=source_language,translated_language=translated_language, style="general")
...
def academic_translate(llm, text: str, source_language: str, translated_language: str, style: str) -> str:
    ...
    # 在知识库中检索相关知识，并加入提示词
    relevant_info = academic_kb.retrieve_relevant_info(text)
    ...
    chain = prompt | llm | StrOutputParser()
    processed_input = gain_userinput(userinput=text, source_language=source_language,translated_language=translated_language, style="academic")
    response = chain.invoke({"input": processed_input})
    return str(response).strip('"\\\'')
...
def literary_translate(llm, text: str, source_language: str, translated_language: str, style: str) -> str:
    ...
    chain = prompt | llm | StrOutputParser()
    processed_input = gain_userinput(userinput=text, source_language=source_language,translated_language=translated_language, style="literary")
    response = chain.invoke({"input": processed_input})
    return str(response).strip('"\\\'')
